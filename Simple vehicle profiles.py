...
vehicles = [
    # Heavy trucks - restricted roads, high capacity
    {
        ...
        "restrictions": {
            ...
            "avoid_ferries": True,
            "avoid_uturns": True,
            "min_road_width": 4.0  # meters - --- not implemented yet
        }
    },
...
def solve_vrp_enhanced(locations, demands, vehicles, compare_profiles=False):
    ...
    Args:
        ...
        demands: List of demands in kgs
        vehicles: Vehicle configuration
        compare_profiles: If True, solve both with and without vehicle profiles
    
    Returns:
        If compare_profiles=False: (routes, distance_matrices, vehicle_specs)
        If compare_profiles=True: (routes_with_profiles, routes_without_profiles, comparison_data)
    """
    
    if compare_profiles:
        print("\n🔄 COMPARISON MODE: Solving with AND without vehicle profiles...")
        
        # Solve WITH vehicle profiles (existing logic)
        print("\n" + "="*60)
...
def _solve_single_vrp(locations, demands, vehicles, use_profiles=True):
    """
    Core VRP solving logic - can be called with or without profiles
    """
    ...
    if use_profiles:
        ...
        truck_distances, truck_durations = get_osrm_matrix_with_profile(
            locations, "driving", vehicles[0]["restrictions"]
        )
        
        print("   Getting driving profile distances...")
...
# ===========================
# 8. ENHANCED RESULTS DISPLAY WITH COMPARISON
# ===========================
...
def display_comparison_results(routes_with, routes_without, comparison_data, locations, demands):
    ...
    if routes_with and routes_without:
        ...
        # Route changes analysis
        route_changes = comp['route_changes']
        
        if route_changes['vehicle_type_changes']:
            print(f"\n🔄 VEHICLE TYPE CHANGES:")
            for change in route_changes['vehicle_type_changes']:
                print(f"   {change['customer']}: {change['without_profiles']} → {change['with_profiles']}")
        
        # Impact assessment
        print(f"\n📈 IMPACT ASSESSMENT:")
        if comp['differences']['cost_percentage'] > 5:
            print("   🔴 Vehicle profiles significantly INCREASE costs (+5%+)")
        elif comp['differences']['cost_percentage'] < -5:
            print("   🟢 Vehicle profiles significantly REDUCE costs (-5%+)")
        else:
            print("   🟡 Vehicle profiles have minimal cost impact (<5%)")
            
        if comp['differences']['distance_percentage'] > 0:
            print(f"   📏 Profiles add {comp['differences']['distance_percentage']:.1f}% extra distance (road restrictions)")
        else:
            print(f"   📏 Profiles reduce distance by {abs(comp['differences']['distance_percentage']):.1f}% (better routing)")
            
        # Recommendations
        print(f"\n💡 RECOMMENDATIONS:")
        if comp['differences']['cost_percentage'] > 10:
            print("   • Consider relaxing vehicle restrictions for cost savings")
            print("   • Evaluate if road restriction compliance is worth the extra cost")
        elif comp['differences']['cost_percentage'] < -10:
            print("   • Vehicle profiles provide significant cost benefits")
            print("   • Recommend using profile-based routing")
        else:
            print("   • Vehicle profiles provide realistic routing with minimal cost impact")
            print("   • Recommend using profiles for compliance and safety")
